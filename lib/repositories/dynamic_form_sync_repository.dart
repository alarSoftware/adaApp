    ...
    try {
      ...
      final jsonToSend = {
        ...
        'details': details.map((d) => {
          ...
          'response': d.response,
          'tieneImagen': d.tieneImagen,
          'imagenBase64': d.imagenBase64,
          'imagenTamano': d.imagenTamano,
  ...
  Future<String?> _saveJsonToTextFile(
    ...
    try {
      ...
      for (var detail in details) {
        ...
        if (detail.tieneImagen) {
          buffer.writeln('      📷 IMAGEN ADJUNTA:');
          buffer.writeln('         • Tamaño: ${detail.imagenTamano} bytes (${(detail.imagenTamano / 1024).toStringAsFixed(2)} KB)');
          buffer.writeln('         • Path: ${detail.imagenPath}');
          buffer.writeln('         • Base64 Length: ${detail.imagenBase64?.length ?? 0} caracteres');
          buffer.writeln('         • Preview (primeros 100 chars):');
          buffer.writeln('           ${detail.imagenBase64?.substring(0, 100) ?? ""}...');
        }
