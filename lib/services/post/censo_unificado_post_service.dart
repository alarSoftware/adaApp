// lib/services/post/censo_unificado_post_service.dart
...
class CensoUnificadoPostService {
  ...
  static Future<Map<String, dynamic>> enviarCensoUnificado({
    ...
    try {
      ...
      // 🔥 GUARDAR LOG TXT SIMPLE (si está habilitado)
      if (guardarLog) {
      ...
      await _logNetworkError('Sin conexión de red', e, censoId, fullUrl, userId);
      return _errorResponse('Sin conexión de red');

    } on TimeoutException catch (e) {
      _logger.e('⏰ Timeout: $e');
      await _logNetworkError('Timeout de conexión', e, censoId, fullUrl, userId);
      return _errorResponse('Tiempo de espera agotado');

    } on http.ClientException catch (e) {
      _logger.e('🌐 Error de cliente HTTP: $e');
      await _logNetworkError('Error de cliente HTTP', e, censoId, fullUrl, userId);
      return _errorResponse('Error de red: ${e.message}');

    } catch (e, stackTrace) {
      _logger.e('❌ Error general: $e', stackTrace: stackTrace);
      ...
      return _errorResponse('Error interno: $e');
    }
  }
  ...
  static String _generarContenidoLogSimple({
    ...
    buffer.writeln('CENSO UNIFICADO - POST REQUEST LOG');
    buffer.writeln(separador);
    ...
    // Resumen del censo
    buffer.writeln(divisor);
    buffer.writeln('RESUMEN DEL CENSO UNIFICADO:');
    buffer.writeln(divisor);
  ...
  static void _agregarResumenSimple(
    ...
    buffer.writeln('Es nuevo equipo: ${equipo != null}');
    buffer.writeln('Crear pendiente: ${pendiente != null}');
  ...
  static Future<File?> _obtenerArchivoLog() async {
    ...
    final fileName = 'censo_unificado_post_$fechaFormateada.txt';
  ...
  /// Construye el payload unificado con los 3 JSONs diferenciados (SOLO)
  static Map<String, dynamic> _construirPayloadUnificado({
    ...
    // 🔥 SOLO LAS 3 SECCIONES QUE NECESITA EL BACKEND
    final Map<String, dynamic> payload = {};

    // ═══════════════════════════════════════════════════════════════
    // SECCIÓN EQUIPO (si es nuevo equipo)
    // ═══════════════════════════════════════════════════════════════
    if (esNuevoEquipo && marcaId != null && modeloId != null && logoId != null) {
      ...
      _logger.i('✅ JSON Equipo agregado al payload');
    }

    // ═══════════════════════════════════════════════════════════════
    // SECCIÓN EQUIPO_PENDIENTE (si se debe crear pendiente)
    // ═══════════════════════════════════════════════════════════════
    if (crearPendiente) {
      ...
      _logger.i('✅ JSON Equipo_Pendiente agregado al payload');
    }

    // ═══════════════════════════════════════════════════════════════
    // SECCIÓN CENSO_ACTIVO (siempre se incluye)
    // ═══════════════════════════════════════════════════════════════
    ...
    _logger.i('✅ JSON Censo_Activo agregado al payload');
    ...
    try {
      ...
      if (responseBody is Map && responseBody.containsKey('serverAction')) {
        ...
        if (serverAction == ServerConstants.SUCCESS_TRANSACTION) {
          _logger.i('✅ Censo unificado procesado correctamente (Action 100)');
          return {
            'exito': true,
            'success': true,
            'mensaje': responseBody['resultMessage'] ?? 'Censo unificado procesado correctamente',
            'serverAction': serverAction,
            'servidor_id': responseBody['resultId'],
            'id': responseBody['resultId'],
          };
        } else {
          _logger.e('❌ Servidor rechazó censo unificado (Action: $serverAction)');
          return {
            'exito': false,
            'success': false,
      ...
      return {
        'exito': true,
        'success': true,
        'mensaje': 'Censo unificado procesado (sin serverAction)',
        'id': responseBody['id'],
      };
  ...
  static Future<void> _logNetworkError(
      String message,
      dynamic error,
      String? censoId,
      String? fullUrl,
